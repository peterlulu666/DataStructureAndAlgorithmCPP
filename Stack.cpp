...
void push(int stack[], int data, int capacity) {
    if (top ==
        capacity - 1)       //If the top position is the last of position of the stack, this means that the stack is full.
    {
        cout << "Stack is full.Overflow condition!";
    } else {
        top = top + 1;            //Incrementing the top position
        stack[top] = data;       //Inserting an element on incremented position
    }
}
